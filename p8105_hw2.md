...
- Because of urgent timing, I’ll use the solution provided as a sample
  in the Problem 1. We clean the 538 `pols` data, which provides
  information on the number of national politicians who are democratic
  or republican at any given time. There are some values for which
  `prez_gop` is `2` – these are months in which Ford became President
  following Nixon’s resignation. In the new `president` variable created
  as part of our data cleaning, we code these as `gop` (same as values
  when `prez_gop` is `1`).
...
```
    ...
    ## #   cigarette_butts <dbl>, plastic_bags <dbl>, homes_powered <dbl>, x12 <lgl>,
    ## #   trash_wheel <chr>
...
```
    ...
    ## #   cigarette_butts <dbl>, glass_bottles <dbl>, plastic_bags <dbl>,
    ## #   wrappers <dbl>, sports_balls <dbl>, homes_powered <dbl>, trash_wheel <chr>,
    ## #   grocery_bags <dbl>, chip_bags <dbl>, x12 <lgl>
...
The number of observations in the resulting dataset is 782.

1.  In particular, the mr_trashwheel dataset has 582 rows and 15
    ...
    and volume are stored, with the mean of 3.2112027 tons of trash each
    day and the volume of 15.3075601 cubic yard each day.
...
3.  The Gwynnda dataset has 106 rows and 13 variables. As the same
